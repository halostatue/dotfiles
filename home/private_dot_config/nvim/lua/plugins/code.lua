...
function Snippets:expander()
  if self.plugin == 'luasnip' then
    return function(args) require('luasnip').lsp_expand(args.body) end
  elseif self.plugin == 'vsnip' then
    return function(args) vim.call('vsnip#anonymous', args.body) end
  elseif self.plugin == 'ultisnips' then
    return function(args) vim.call('UltiSnips#Anon', args.body) end
  elseif self.plugin == 'snippy' then
    return function(args) require('snippy').expand_snippet(args.body) end
  end
end
...
function Snippets:source()
  if
    self.plugin == 'luasnip'
    or self.plugin == 'vsnip'
    or self.plugin == 'ultisnips'
    or self.plugin == 'snippy'
  then
...
return {
  ...
  { -- https://github.com/preservim/tagbar
    ...
    config = function() vim.g.tagbar_compact = 1 end,
  },
  ...
  { -- https://github.com/tpope/vim-endwise
    'tpope/vim-endwise',
    event = { 'InsertEnter' },
    init = function() vim.g.endwise_abbreviations = 1 end,
  },
  { -- https://github.com/tpope/vim-ragtag
    'tpope/vim-ragtag',
    event = { 'InsertEnter' },
    init = function() vim.g.ragtag_global_maps = 1 end,
  },
